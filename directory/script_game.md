...
**Bust popup:** `_show_bust_popup(roller_id)` sets `BustLabel` text to `"You Busted!"` for the local player or `"Player X Busted!"` for others before showing the panel. Each peer resolves this independently from the `roller_id` already broadcast by `_sync_roll_result`.
